...
void usage(const char* fullpath)
{
    ...
    printf(
        "Usage:\n"
        "  %s [-aXXXX] [-b] [-x] [-fXX] [-c] [-oXXXX] [-sXXXX] <infile>\n"
...
// *** enable_raw_mode, disable_raw_mode, kbhit ***
// https://stackoverflow.com/questions/29335758/using-kbhit-and-getch-on-linux
void enable_raw_mode() {
    termios term;
    tcgetattr(0, &term);
    term.c_lflag &= unsigned( ~(ICANON | ECHO) ); // Disable echo as well
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1, j = 0; i < argc; i++) {
        if ('-' == argv[i][0]) {
            switch (argv[i][++j]) {
                case 'a':             // IODATA / IOSTAT reg address
                    if (argv[i][++j]) // "-aXXXX"
                        result = sscanf(argv[i] + j, "%x", &IOSTAT);
                    else if (i < argc - 1) // "-f XX"
                        result = sscanf(argv[++i], "%x", &IOSTAT);
                    if (result) {
                        IODATA = IOSTAT >> 8; // HIGH part
                        IOSTAT &= 0xFF;       // LOW part
                    }
                    else {
                        fprintf(stderr, "Error: option -a needs a hexadecimal argument\n");
                        return 1;
                    }
    ...
    Z80 z80( // create processor object with memory and IO callback functions
        ...
        // input one byte from IO space to Z80
        [](void* arg, unsigned short port) {
            ...
            return '\0'; // IO[port & IO_MASK];
        },

        // output one byte from Z80 to IO space
        ...
        false  // false: provide 8 bit IO addresses
    );
    ...
    z80.reg.PC = start;
    // switch to char input w/o ENTER, no echo, convert \n -> \r
